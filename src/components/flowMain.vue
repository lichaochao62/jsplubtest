<template>
  <div class="appcontainer">
    ...
    <div v-show="editlinename" class="popwindow">
    <input v-model="linetext" />
    <button @click="addlinetext" >确定</button>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      addlinetext()
      {
        ...
        let connection = this.currentConnect
        if(typeof this.currentConnect.source === 'undefined'){
          console.log('新建线路')
          connection = this.jsPlumb.connect({
            source: this.currentConnect.sourceId,
            target: this.currentConnect.targetId,
            uuids: this.getUUID()
          }, this.jsplumbConnectOptions);
        }
          connection.getOverlay("label-1").setLabel(this.linetext); //初始化label
      },
      _editLabel(info)
      {
         console.log(info)
